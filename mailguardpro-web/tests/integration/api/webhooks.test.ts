...
describe('/api/v1/webhooks', () => {
  ...
  describe('GET', () => {
    ...
    it('should return webhooks for authenticated user', async () => {
      ...
      vi.mocked(prisma.webhook.findMany).mockResolvedValue([
        { id: 'webhook-1', url: 'https://example.com/hook1', events: ['bulk_job_completed'], isActive: true },
        { id: 'webhook-2', url: 'https://example.com/hook2', events: ['credit_low'], isActive: false },
      ])
  ...
  describe.skip('POST', () => {
    it('should return 401 when not authenticated', async () => {
      ...
      const body = { url: 'https://example.com/hook', events: ['bulk_job_completed'] }
      const req = new NextRequest('http://localhost:3000/api/v1/webhooks', {
        method: 'POST',
    ...
    it('should create webhook with valid data', async () => {
      ...
      const body = { url: 'https://example.com/hook', name: 'My Webhook', events: ['bulk_job_completed'] }
      const req = new NextRequest('http://localhost:3000/api/v1/webhooks', {
        method: 'POST',
        body: JSON.stringify(body),
      })
