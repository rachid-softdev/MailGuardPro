...
import { checkMX, checkSPF, checkDMARC } from '@/services/dnsChecker'
...
describe('emailValidator', () => {
  beforeEach(() => {
    ...
    vi.mocked(checkMX).mockResolvedValue({ passed: true, weight: 25, message: 'MX OK' })
    vi.mocked(checkSPF).mockResolvedValue({ passed: true, weight: 5, message: 'SPF OK' })
    vi.mocked(checkDMARC).mockResolvedValue({ passed: true, weight: 5, message: 'DMARC OK' })
    vi.mocked(checkSMTP).mockResolvedValue({ passed: true, weight: 30, message: 'SMTP OK' })
    vi.mocked(checkDisposable).mockResolvedValue({ passed: true, weight: 10, message: 'Not disposable' })
    vi.mocked(checkGeneric).mockResolvedValue({ passed: true, weight: 5, message: 'Not generic' })
    vi.mocked(checkFreeProvider).mockReturnValue({ passed: true, weight: 0, message: 'Business email' })
    vi.mocked(checkTypo).mockReturnValue({ passed: true, weight: 0, message: 'No typo' })
    vi.mocked(checkDNSBL).mockResolvedValue({ passed: true, weight: 0, message: 'Not blacklisted' })
  ...
  describe('validateEmail', () => {
    ...
    it('should return invalid status for failed format check', async () => {
      vi.mocked(checkFormat).mockReturnValue({ passed: false, weight: 15, message: 'Invalid format' })
    ...
    it('should return invalid status for disposable email', async () => {
      vi.mocked(checkDisposable).mockResolvedValue({ passed: false, weight: 10, message: 'Disposable' })
    ...
    it('should handle typo without affecting valid status when SMTP passes', async () => {
      vi.mocked(checkTypo).mockReturnValue({ passed: false, weight: 0, message: 'Typo detected' })
