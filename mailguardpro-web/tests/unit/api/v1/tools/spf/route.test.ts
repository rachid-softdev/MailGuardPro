...
import { GET } from "@/app/api/v1/tools/spf/route";

const BASE = "http://localhost/api/v1/tools/spf";

describe("GET /api/v1/tools/spf (P1/P0 SSRF)", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns 400 when domain is an IP literal (SSRF guard)", async () => {
    const res = await GET(get("1.2.3.4"));
    expect(res.status).toBe(400);
  ...
  it("returns hasSpf true when an SPF record is present", async () => {
    mockResolveTxt.mockResolvedValue([["v=spf1 include:_spf.google.com ~all"]]);
    const res = await GET(get("example.com"));
    const body = await res.json();
    expect(body.data.hasSpf).toBe(true);
    expect(body.data.spfRecord).toContain("v=spf1");
  });

  it("returns hasSpf false when no SPF record exists", async () => {
    mockResolveTxt.mockResolvedValue([["google-site-verification=xyz"]]);
    const res = await GET(get("example.com"));
    const body = await res.json();
    expect(body.data.hasSpf).toBe(false);
    expect(body.data.spfRecord).toBeNull();
  });
